...
i2c_inst_t *ssd1306::I2C::detect_instance(uint pin_sda, uint pin_scl) {
    invalid_params_if(I2C, (pin_sda & 2) != (pin_scl & 2));
    return i2c_get_instance((pin_sda >> 1) & 1);
}

void ssd1306::I2C::init_io(uint baudrate) {
...
spi_inst_t *ssd1306::SPI::detect_instance(uint sda_pin, uint scl_pin) {
...
void ssd1306::SPI::init_io(uint baudrate) {
...
void ssd1306::SPI::write_internal(const uint8_t *data, const size_t length, bool is_data) {
    gpio_put(dc, is_data);
    gpio_put(cs, 0);
